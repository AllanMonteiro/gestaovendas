...
class CashDashboardConfigFallbackTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_superuser(email='cash-dashboard@test.com', password='test123', name='Cash Dashboard Admin')
        self.client.force_authenticate(user=self.user)

    def test_dashboard_returns_defaults_when_store_config_load_fails(self):
        fallback_summary = {
            'total_sales': None,
            'total_orders': 0,
            'avg_ticket': None,
        ...
        with patch('apps.sales.views.services.get_store_config', side_effect=OperationalError('no such column')), \
             patch('apps.sales.views.report_queries.summary', return_value=fallback_summary):
            response = self.client.get('/api/cash/dashboard?from=2026-05-01&to=2026-05-01')

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data['config']['store_name'], 'Sorveteria POS')
        self.assertEqual(response.data['config']['company_name'], None)
        self.assertEqual(response.data['config']['cnpj'], None)
        self.assertEqual(response.data['config']['address'], None)
        self.assertEqual(response.data['config']['printer'], {})
        self.assertEqual(response.data['payments'], [])


class OrdersCanceledViewTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_superuser(email='orders-canceled@test.com', password='test123', name='Orders Canceled Admin')
        self.client.force_authenticate(user=self.user)
        self.order = Order.objects.create(
            status=Order.STATUS_CANCELED,
            subtotal=Decimal('12.00'),
            total=Decimal('12.00'),
            canceled_reason='Cliente desistiu',
        )

    def test_canceled_orders_accepts_date_range_without_500(self):
        response = self.client.get('/api/orders/canceled?from=2026-05-01&to=2026-05-01&include_items=0&limit=50')
