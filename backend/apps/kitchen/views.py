...
class KitchenReadyView(APIView):
    def post(self, request, order_id):
        ...
        broadcast_pdv_event('order_ready', {'order_id': str(order_id)})
...
class KitchenBackToPrepView(APIView):
    def post(self, request, order_id):
        ...
        Order.objects.filter(id=order_id).update(status=Order.STATUS_SENT)
        broadcast_pdv_event('order_status_changed', {'order_id': str(order_id), 'status': Order.STATUS_SENT})
